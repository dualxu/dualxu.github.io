...
于是我又想到我上个月写的辅助方法 `UIDispatcher.RunNewAsync()`，在后台创建一个运行起来的 `Dispatcher`。参见我博客 [如何实现一个可以用 await 异步等待的 Awaiter - dualxu](https://dualxu.gitee.io/post/write-custom-awaiter.html) 中 `UIDispatcher` 的实现。
...
- [深入了解 WPF Dispatcher 的工作原理（PushFrame 部分） - dualxu](https://dualxu.github.io/post/dotnet/2017/09/26/dispatcher-push-frame.html)
...
*`Yield` 的意思可以参见我的另一篇博客 [出让执行权：Task.Yield, Dispathcer.Yield - dualxu](https://dualxu.github.io/post/yield-in-task-dispatcher.html)。*
